...
//
// 			expect(rootChildNodes.length).toEqual(columns.length);
// 			// Verify columns
...
//
// 				expect(columnChildNodes.length).toEqual(columns[ i ].items.length + 1, `Column data: ${JSON.stringify(columns[ i ].items)} Rendered: ${columnRoot.innerHTML}`);
// 				expect(columnRoot.firstChild.innerHTML).toEqual('column', 'Column first child check');
//
// 				// Verify items
// 				// Skip first - its hardcoded
// 				for (let j = 1; j < columnChildNodes.length; j++) {
// 					const itemRoot = columnChildNodes[ j ];
// 					expect(itemRoot.innerHTML).toEqual(columns[ i ].items[ j - 1 ].text.toString(), 'item content check');
// 				}
// 			}
// 		}
//
...
// 					expect(mountedColumnSpy.callCount).toEqual(testCase.initial.length, 'Column Initial MOUNT'); // Initial all mounted
// 					expect(unmountColumnSpy.callCount).toEqual(0, 'Column Initial unMount'); // Initial render none unmounted
// 					expect(updateColumnSpy.callCount).toEqual(0, 'Column Initial update'); // Initial render none to update
//
// 					expect(mountedItemSpy.callCount).toEqual(initialItemsCount, 'Item Initial Mount'); // Initial render - mount all items once
// 					expect(updateItemSpy.callCount).toEqual(0, 'Item initial update'); // Initial render none to update
// 					expect(unmountItemSpy.callCount).toEqual(0, 'Item initial unmount'); // Initial render none unmounted
//
// 					// reset call counts
...
//
// 					expect(mountedColumnSpy.callCount).toEqual(columnsToBeAdded.length, 'mount'); // mount count should equal to added count
// 					expect(unmountColumnSpy.callCount).toEqual(columnsToRemove.length, 'unmount'); // Initial render none unmounted
// 					expect(updateColumnSpy.callCount).toEqual(columnsToUpdate.length, 'update'); // Initial render none unmounted
// 					expect(mountedItemSpy.callCount).toEqual(itemsToBeAdded.length, `itemsToBeAdded ${JSON.stringify(itemsToBeAdded)} componentWillMount called: ${mountedItemSpy.callCount} times.`); // Initial render - mount all items once
// 					expect(updateItemSpy.callCount).toEqual(itemsToUpdate.length, 'item update callback count'); // Initial render none to update
// 					expect(unmountItemSpy.callCount).toEqual(itemsToRemove.length, 'item unmount callback count'); // Initial render none unmounted
// 				});
// 			});
// 		});
//
