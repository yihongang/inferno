import { InfernoChildren, render, IVNode } from 'inferno';
...
export function isVNodeOfType(instance: IVNode, type: string | Function): boolean {
...
export function isFunctionalVNodeOfType(instance: IVNode, type: Function): boolean {
...
export function renderIntoDocument(input): InfernoChildren {
...
export function findAllInRenderedTree(renderedTree: any, predicate: (vNode: IVNode) => boolean): IVNode[] | any {
...
export function findAllInVNodeTree(vNodeTree: IVNode, predicate: (vNode: IVNode) => boolean): any {
	if (isVNode(vNodeTree)) {
		...
		if (isRenderedClassComponent(children)) {
			result = result.concat(findAllInVNodeTree(children._lastInput, predicate) as IVNode[]);

		} else if (isVNode(children)) {
			result = result.concat(findAllInVNodeTree(children, predicate) as IVNode[]);

		} else if (isArray(children)) {
			(children as any[]).forEach((child) => {
				result = result.concat(findAllInVNodeTree(child, predicate) as IVNode[]);
			});
		}
		return result;
	} else {
...
export function scryRenderedVNodesWithType(renderedTree: any, type: string | Function): IVNode[] {
	return findAllInRenderedTree(renderedTree, (instance) => isVNodeOfType(instance, type));
}

export function scryVNodesWithType(vNodeTree: IVNode, type: string | Function): IVNode[] {
...
export function findRenderedVNodeWithType(renderedTree: any, type: string | Function): IVNode {
	return findOneOf(renderedTree, type, 'component', scryRenderedVNodesWithType);
}

export function findVNodeWithType(vNodeTree: IVNode, type: string | Function): IVNode {
